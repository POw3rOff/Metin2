...
A função **guild.high_ranking_string** informa топ гильдий по рангу.

## Valores de retorno
### high_ranking
Tipo *string*. Список из 8 гильдий, которые имеют наибольшее количество очков ранга. Каждая гильдия размещается на новой строке, перенос строк осуществляется выражением `[ENTER]`. Строка выглядит примерно так: `{rank} | {name} | {ladder_points} | {win} | {draw} | {loss}[ENTER]...`

* `{rank}` &mdash; это позиция гильдии;
* `{name}` &mdash; o nome da guilda;
...
Se гильдия, в ao qual состоит игрок, находится в этом списке, então строка будет дополнена выражением `[COLOR r;255|g;255|b;0]`, которое будет добавлено в начало строки с гильдией.
