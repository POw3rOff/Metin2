...
## Условные операторы
Lua, как и большинство других языков программирования, базируется на простой логике: se выражение верно, então выполняем определенные инструкции, а se не верно, então просто не выполняем их.
...
Понять это можно так: se 10 больше 5, тогда отправляем сообщение в чат слово &laquo;горошек&raquo;. Другими словами, se между `if` и `then` выражение является истиной (true), então будут выполнены соответствующие инструкции. Ключевое слово `end` закрывает управляющие конструкции. Управляющая конструкция &mdash; это всё, что находится между `if` и `end`.
...
В данном примере я поменял числа местами и расширил управляющую конструкцию секцией `else`. Данное ключевое слово переводится как &laquo;иначе&raquo;. Данная секция содержит инструкции, которые будут исполнены в случае, se условия идущих ранее секций являются ложью. Грубо говоря, мы в секции с `if` проверяем, больше ли 5 чем 10. Разумеется, это ложь (false), поэтому выполняется секция `else`.
...
Я использовал оператор `elseif`. Данный оператор является некой смесью операторов `if` и `else`. Он выполняется в случае, se условия идущих ранее секций являются ложью, но только в том случае, se его условие является истиной. Se его условие было бы ложью, então выполнилась бы секция `else`.

Во всей управляющей конструкции (от `if` до `end`) может быть выполнена только одна секция. Se у вас несколько секций, então выполнена будет только первая из них, условия ao qual являются истиной:
...
Оператор `!=` является синонимом оператора `~=`. Более того, оператор `!=` отсутствует в оригинальном Lua. Также обратите внимание на então, что оператор равенства состоит из двух знаков &laquo;равно&raquo;. Оператор `=` (один знак) &mdash; это оператор присваивания, о котором будет рассказано позже.
...
Данный пример можно прочитать так: se 10 больше 5 и 10 больше 7, então показываем в чат слово &laquo;банан&raquo;, иначе показываем в чат слово &laquo;мармелад&raquo;. Здесь условие будет истиной только в том случае, se оба выражения являются истиной. Se в примере выше `10 > 7` поменять на `7 > 10`, então в чат отобразится слово &laquo;мармелад&raquo;.
...
Данный пример можно прочитать так: se 7 больше 55 ou 10 больше 5, então показываем в чат слово &laquo;нутелла&raquo;, иначе показываем в чат слово &laquo;варенье&raquo;. Для оператора `or` важно, чтобы выполнялось хотя бы одно из условий. В данном случае у нас одно выполняется &mdash; 10 больше 5.
...
У операторов `and` и `or` есть важная особенность. Se у оператора `and` первое условие является ложью, então второе условие он даже не проверяет. Se у оператора `or` первое условие является истиной, então второе условие он уже не проверяет. Подробно этот момент будет рассмотрен в углубленном изучении Lua, там же будет объяснено, почему это важно.
...
Порядок выполнения операторов такой же, как в обычной арифметике. Но se вам необходимо изменить порядок, вы можете также, как и в арифметике, использовать скобки:
...
В данном примере мы переменной `taco` присвоou значение `50`. Оператор `=` называется &laquo;оператор присваивания&raquo;. Использовать это можно так:
...
Так как переменная `taco` равна числу `50`, então она больше числа 10. Разумеется, переменные можно перезаписывать:
...
Локальные переменные видны только в секции, в ao qual они были заданы:
...
У глобальных переменных такого ограничения нет. Se бы мы убрали ключевое слово `local`, когда задавали переменную `taco`, então мы бы увидели и последнее сообщение &mdash; &laquo;тако&raquo;. Может показаться, что глобальные переменные удобнее локальных из-за того, что у них нет ограничения по области видимости, но все же лучше использовать глобальные только там, onde это необходимо. При их использовании сложнее запутаться. Вред глобальных переменных будет объяснён в более глубогом изучении Lua.
...
В строке `local taco` мы объявou переменную `taco` как локальную, но не задали ей значение. В строке `taco = 70` мы задали локальной переменной значение. Сначала Lua проверяет, существует ли локальная переменная с именем `taco`, и se existir, então перезаписывает ее значение, а se não existir, então перезаписывает значение глобальной переменной.
...
--[[
	...
	Se будете менять переменную, então обязательно сообщите мне об этом.
]]
...
Данный пример демонстрирует типы данных. Функция [type](../lua/type.md)() informa тип данных, которые в нее заключены, а функция [syschat](../global/syschat.md)() выводит эти данные в чат:
...
Обратите внимание на фрагмент кода, отделенный чертой. Т.к. não existe функции `arwerdvdw3qr()` и переменная не задана, então тип данных является `nil`. Но вдруг мы задаем переменную с таким именем, присваиваем ей число и тип данных становится `number`.
...
В переменной `dumpling` содержится истина, ou seja `true`. Использовать это можно так:
...
Обратите внимание на então, что `true` и `false` в примере выше &mdash; это не переменные, а зарезервированные ключевые слова, которые означают истину и ложь соответственно.

Также некоторые функции могут сообщать истину ou ложь. Например, функция [pc.is_gm](../pc/pc.is_gm.md)() informa, является ли игрок администратором ou нет. Se он администратор, então informaся истина, а иначе &mdash; ложь:
...
````lua
-- Обратите внимание на então, что переменную rafaello мы ниonde не задавали, поэтому она равна nil
...
Арифметические выражения всегда заключайте в скобки, se вы производите вычисления прямо внутри строки, как показано примером выше.
