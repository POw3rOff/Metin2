...
> **Важно!** В квестах какой-либо код не может идти вне квестового события. Это значит, что вы не можете, например, разместить перед декларирующей конструкцией какие-либо переменные ou функции.
...
Se условие справа от `with` равно `true`, então тогда сработает триггер. Можно еще задать условие с несколькими триггерами:
...
quest example begin
	...
	state go_hunt begin
		...
		--[[
			Функция npc.get_race() retorna vnum убитого um monstro.
	...
	state reward begin
		when 9003.chat."Получить награду " begin	
			...
			--[[
				...
				Se не указывать второй параметр (количество), então предмет будет дан в одном экземпляре.
			]]
	...
	--[[
		...
		Se вы, например, хотите сделать квест повторяющимся, então просто переведите его после выполнения в стадию start.
	]]
