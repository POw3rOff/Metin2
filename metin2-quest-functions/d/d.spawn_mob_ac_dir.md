# d.spawn_mob_ac_dir()
A função **d.spawn_mob_ac_dir** invoca um monstro ou NPC na masmorra, повернутого в определенную сторону на определенное количество градусов.
...
### direction
Tipo *number*. **Parâmetro obrigatório**. Направление в градусах по часовой стрелке, куда будет смотреть монстр. Направление может быть от `0` до `359`. Se параметр равен `-1`, então направление будет задано случайно.
