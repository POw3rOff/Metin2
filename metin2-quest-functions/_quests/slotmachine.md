# Слот-машина
Данный квест является некой эмуляцией слот-машины. Суть такая: есть ряд из 5 ячеек. Когда в этих ячейках появляются 3 предмета ou больше подряд, então игрок побеждает. Вот пример выигрышной комбинации:
...
### slotmachine.quest
Создайте файл с именем `slotmachine.quest`, задайте ему кодировку ANSI ou Windows-1251 (также известна как cp1251) и поместите туда код:
...
quest slotmachine begin
	state start begin
		when SLOTMACHINE__NPC_VNUM.chat."Сыграть в игру " begin
			...
			say("Привет! Правила игры посты: se ")
			say("тебе выпадет 3 предмета в ряд, então ")
			say("ты их и получишь. Se впадет 4 ou ")
			say("5 предметов, então награда увеличится. ")
			say(" ")
...
Константа **SLOTMACHINE__WRITE_LOGS** отвечает за então, нужно ли сохранять логи побед в базе данных. `true`, se нужно, иначе `false`. Логи можно найти в таблице `log.log`. Поле `how` всегда содержит `SLOTMACHINE`; поле `what` &mdash; vnum предмета, который выиграл игрок; поле `hint` &mdash; количество предметов, которое выиграл игрок; поле `who` &mdash; ID персонажа игрока.<br>
